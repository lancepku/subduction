...
do i=1,nx
    do j=1,nz
        ...
        if( ncod(j,i,2) .eq. 1 ) then   !!!  It is where the velocity bc are set
            !vel(j,i,2) = bc(j,i,2)
             !! here calculate gradient from position of virtual external boundary
              vel(j,i,2) = (vel(j-1,i,2)-vel(j,i,2))*(cord(j-1,i,2)-cord(j,i,2))/delzvirtual
        else
